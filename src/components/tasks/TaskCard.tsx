...
const TaskCard: React.FC<ITaskCardProps> = ({ task }) => {
  ...
  return (
    <motion.div 
      ...
      className={`group relative glass-premium border-obsidian rounded-outer p-8 transition-all duration-500 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.6)] flex flex-col h-full ${
        task.status === "Done" ? "opacity-50 saturate-[0.5]" : ""
      }`}
    >
      {/* Dynamic Background Glow */}
      <div className={`absolute top-0 right-0 w-64 h-64 blur-[120px] opacity-10 transition-opacity group-hover:opacity-25 pointer-events-none ${
        task.priority === 'High' ? 'bg-red-500' : task.priority === 'Medium' ? 'bg-amber-500' : 'bg-indigo-500'
      }`} />
      
      <div className="flex justify-between items-start mb-6 gap-4">
        <h3 className="text-2xl font-black text-white line-clamp-2 leading-[1.1] tracking-tighter flex-1">
          {task.title}
        </h3>
        <div className="flex flex-col items-start gap-3 shrink-0">
          <div className="flex items-center gap-2">
            <motion.button 
              ...
              className="p-2 text-zinc-600 hover:text-red-400 hover:bg-red-400/10 rounded-inner transition-colors bg-white/5 border border-white/5"
            >
          ...
          {task.estimatedTime && (
            <div className="flex flex-col items-start gap-2">
               <div className="flex items-center gap-2.5 bg-white/5 border border-white/10 px-4 py-2 rounded-inner shadow-inner group/time">
      ...
      <div className="flex items-center gap-4 mb-6">
        <motion.button
          ...
          disabled={isUpdating}
          className={`px-5 py-2 rounded-inner text-[11px] font-black uppercase tracking-widest flex items-center gap-2.5 transition-all shadow-xl ${
        ...
        {task.estimatedTime && task.estimatedTime > 240 && (
          <div className="flex items-center gap-2.5 px-4 py-2 bg-red-500/10 text-red-400 border border-red-500/20 rounded-inner text-[10px] font-black uppercase tracking-tight animate-pulse shadow-inner">
      ...
      {task.tags && task.tags.length > 0 && (
        <div className="flex flex-wrap gap-2.5 mb-8">
          {task.tags.map((tag: string, i: number) => (
            <div key={`${task._id}-tag-${i}`} className="flex items-center gap-2 px-3 py-1 rounded-inner bg-white/5 border border-white/5 text-[10px] font-black text-slate-400 hover:text-indigo-400 hover:border-indigo-500/30 transition-all cursor-default uppercase tracking-wider">
      ...
      <div className="space-y-3 mb-8 flex-1">
        {task.subTasks?.slice(0, 3).map((st: any) => (
          <motion.div 
            key={st._id} 
            whileHover={{ x: -4 }}
            className="group/item flex items-center gap-4 text-[14px] text-zinc-300 cursor-pointer bg-white/[0.02] p-3 rounded-inner border border-transparent hover:border-white/5 transition-all"
          ...
          >
            <div className="flex items-center justify-between w-full flex-row">
              ...
              {st.estimatedTime && (
                <div className="flex flex-row items-center gap-2 px-2.5 py-1 bg-black/20 border border-white/5 rounded-lg">
        ...
        {task.subTasks?.length > 3 && (
          <p className="text-[11px] text-slate-500 font-black uppercase tracking-widest pl-4 flex flex-row items-center gap-2">
      ...
      <div className="flex items-center justify-between mt-auto pt-6 border-t border-white/5">
        <div className="flex flex-row items-center gap-3 text-xs text-slate-400 font-black uppercase tracking-tight">
        ...
        <motion.button
          ...
          className="flex flex-row items-center gap-3 text-xs font-black text-indigo-400 transition-all bg-indigo-500/10 px-5 py-2.5 rounded-inner border border-indigo-500/20 shadow-lg uppercase tracking-wide"
        >
          {loading ? (
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            <div className="mt-6 p-6 bg-indigo-500/10 rounded-outer border border-indigo-500/20 shadow-inner relative">
